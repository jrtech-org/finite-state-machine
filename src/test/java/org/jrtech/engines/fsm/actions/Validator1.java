...
public class Validator1 extends AbstractAlternableEventsAction {

    private static final long serialVersionUID = -6999490051576744056L;
    ...
    public String run(ProcessContext processContextData) {
        ...
        if (inputValueToCheck.toLowerCase().contains("fatal error 1")) {
            processContextData.setContextData(validationResultContextKey, "Validation 1 Error 1");
            effectiveOutputEvent = getAlternateEvents().get(0);
        } else if (inputValueToCheck.toLowerCase().contains("fatal error 2 ")) {
            processContextData.setContextData(validationResultContextKey, "Validation 1 Error 2");
